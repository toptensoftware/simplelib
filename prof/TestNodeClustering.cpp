...
namespace
{
	...
	NodeClustering::Cluster* FindClusterContaining(NodeClustering::Plan* plan, NodeClustering::INode* n)
	{
		for (int i = 0; i < plan->clusters.GetCount(); i++)
		{
	...
	bool IsPlanAcyclic(NodeClustering::Plan* plan)
	{
		Map<NodeClustering::Cluster*, int> remaining;
		List<NodeClustering::Cluster*> ready;
		for (int i = 0; i < plan->clusters.GetCount(); i++)
		{
			auto* c = plan->clusters[i];
