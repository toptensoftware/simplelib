...
// CowListWops<T> ("with operations") - a copy-on-write vector for the
// specific UI-thread/audio-thread hand-off pattern used throughout
// Cantabile's execution graph. This is NOT a general purpose thread-safe
// container - it has exactly one intended usage pattern and relies on that
// pattern for its correctness:
//
...
//     state of the vector as a CowListWopsSnapshot<T>. GetSnapshot() must
//     likewise only ever be called from that one thread - it is not safe to
//     call it concurrently from two threads.
//
...
//
// If you don't need the inserted/deleted tracking, use CowList<T> instead -
// it has the same functionality minus this bookkeeping, and doesn't require
// T to be == comparable.
//
...
// CowListWops is designed for small POD types / raw pointers, not owning
// objects. T must also be == comparable, since the cancel-out reconciliation
// in GetSnapshot() needs it unconditionally (Find/IndexOf/Remove require it
// too, but only if actually called).

template <typename T>
class CowListWops;

template <typename T>
class CowListWopsSnapshot
{
public:
	int GetCount() const
	{
...
private:
	CowListWopsSnapshot(int count, int inserted, int deleted) :
		count(count),
	...
	// CowListWops::m_retired) - GetSnapshot()'s own retire step isn't atomic
	...
	CowListWopsSnapshot<T>* next = nullptr;
	...
	// Only ever called from GetSnapshot()'s reconciliation step.
...
class CowListWops
{
	static_assert(std::is_trivially_copyable_v<T>,
		"T must be trivially copyable for use in CowListWops");

public:
	CowListWops(int initialCapacity = 16, int growCapacity = 16)
	{
		...
		m_current = new CowListWopsSnapshot<T>(0, 0, 0);
	...
	void ReclaimRetired()
	{
		CowListWopsSnapshot<T>* node = m_retired.Set(nullptr);
		while (node)
		{
			CowListWopsSnapshot<T>* next = node->next;
			delete node;
			node = next;
		}
	}
	...
	int InsertAt(int iPosition, const T& val)
	{
		...
		if (m_iUpdateDepth == 0)
		{
			CowListWopsSnapshot<T>* pending = m_pending.Get();
			CowListWopsSnapshot<T>* newSnapshot = BuildSnapshot(pending, &val, 1, nullptr, 0);
	...
	void RemoveAt(int iPosition)
	{
		...
		{
			CowListWopsSnapshot<T>* pending = m_pending.Get();
			CowListWopsSnapshot<T>* newSnapshot = BuildSnapshot(pending, nullptr, 0, &val, 1);
	...
	void Move(int iFrom, int iTo)
	{
		...
		{
			CowListWopsSnapshot<T>* pending = m_pending.Get();
			CowListWopsSnapshot<T>* newSnapshot = BuildSnapshot(pending, nullptr, 0, nullptr, 0);
...
public:
	...
	void EndUpdate()
	{
		...
		if (m_bBatchDirty)
		{
			CowListWopsSnapshot<T>* newSnapshot = BuildSnapshot(m_pBatchBaseline,
	...
	CowListWopsSnapshot<T>& GetSnapshot()
	{
		CowListWopsSnapshot<T>* pending = m_pending.Set(nullptr);
		if (pending)
		{
			...
			CowListWopsSnapshot<T>* oldCurrent = m_current;
			CowListWopsSnapshot<T>* head = m_retired.Get();
			for (;;)
			{
				oldCurrent->next = head;
				CowListWopsSnapshot<T>* prevHead = m_retired.CompareExchange(oldCurrent, head);
				if (prevHead == head)
					break;
				head = prevHead;
			}
...
private:
	...
	CowListWopsSnapshot<T>* BuildSnapshot(CowListWopsSnapshot<T>* pending,
		...
		const T* deletedItems, int deletedItemsCount)
	{
		...
		CowListWopsSnapshot<T>* newSnapshot = new CowListWopsSnapshot<T>(m_iCount, insertedCount, deletedCount);
	...
	void StorePendingSnapshot(CowListWopsSnapshot<T>* newSnapshot, CowListWopsSnapshot<T>* oldSnapshot,
		const T* insertedItems, int insertedItemsCount,
		const T* deletedItems, int deletedItemsCount)
	{
		// Try to store it
	...
	Atomic<CowListWopsSnapshot<T>*> m_pending;

	// Current snapshot of the vector used by reader side
	CowListWopsSnapshot<T>* m_current;
	...
	Atomic<CowListWopsSnapshot<T>*> m_retired;
	...
	int m_iUpdateDepth;
	CowListWopsSnapshot<T>* m_pBatchBaseline;
	bool m_bBatchDirty;
	T* m_pBatchInserted;
